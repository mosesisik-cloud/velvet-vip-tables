...
import { t, applyLang, bootLang, LANGS, getLang, currentLang } from "./i18n.js?v=150";
import { publicFields as mrzPublic, nameMatch, ageYears } from "./mrz.js?v=150";
import { readPassportMrz, jpegFromFile, snapshotVideo, captureStill, focusAt, startCamera, stopCamera, waitForVideo, warmupOcr } from "./passport-ocr.js?v=150";
import { loadFaceApi, detectPassportFace, watchBlink, stopLiveness, requestLivenessTap, matchFaces, facePayload, warmupFaceApi } from "./face-idv.js?v=150";
...
function renderHome() {
  ...
  <section class="section home-restaurants" id="home-restaurants-section">
    <div class="section-head">
      <div><h2>${esc(t("navRestaurants"))}</h2><div class="sub" id="home-restaurants-count">Restauranger i 39 destinationer</div></div>
...
async function mountHomeRestaurants() {
  ...
  const total = Object.values(rows).reduce((sum, destination) => sum + (destination.restaurants || []).length, 0);
  const counter = document.getElementById("home-restaurants-count");
  if (counter && total) counter.textContent = `${total.toLocaleString(currentLang())} restauranger i ${Object.keys(rows).length} destinationer`;
